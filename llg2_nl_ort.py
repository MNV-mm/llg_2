...
F = dot(w,(v-m)/Dt-al*cross(m,(v-m)/Dt))*dx + dot(w,cross(v,h_rest(v,pp,e_f,dedz_v,M_s*M_s/2/kp*phi,M_s*M_s/2/kp*(hd_ext + Hd_v_y), ku, kp, kc, Nu, Np, at)))*dx - dot_v(v,v,w,pp,e_f)*dx
Jac = derivative(F,v)
diffr = Function(FS)
Hd = Function(FS)
...
while j <= 10:
    ...
    solve(F==0, v, BC, J=Jac, solver_parameters = { "newton_solver": { "absolute_tolerance": 1e-13, "relative_tolerance": 1e-12}}) # BC!!!
